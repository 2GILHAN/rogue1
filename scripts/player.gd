...
##
## 집는 동작(PUSH_TIME)이 끝나고도 조금 남게 잡았습니다. 손에 붙는 것을 보고
## 나서 던질 수 있어야 "집었다 -> 던졌다" 가 두 동작으로 읽힙니다.
const HOLD_MIN := 0.30
var _hold_min := 0.0
...
func _swing_move_scale() -> float:
	...
	## 목소리 1.05초까지 끌면 재사용 대기(0.55초)보다 길어져, 연타하면 발이
	## 계속 묶입니다 - 예전에 조작이 끊긴다고 하신 그 상태로 돌아갑니다.
...
func _drive_pose_layer(delta: float) -> void:
	...
	if _bound_time > 0.0:
		...
		# (_try_attack), 소품을 들고 있을 때는 두 팔이 어디로 갈지 하나로
