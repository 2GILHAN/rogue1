...
## 고함 소리에서 **목소리가 살아 있는 길이**(초).
##
## 파일은 1.05초인데 그 값을 쓰면 어긋납니다. 봉우리 크기의 절반 아래로
## 떨어지는 때를 재 보니 **0.86초**였습니다 - 뒤의 0.19초는 잦아드는 꼬리라,
## 귀로는 이미 끝난 소리입니다. 파일 길이에 맞추면 소리가 끝난 뒤에도 부채꼴이
## 계속 자라서 "언제까지 눌러야 하나" 를 귀로 알 수 없습니다.
##
##     실측(10ms 봉우리 포락선):
##       소리가 나는 구간   0.00 ~ 1.03초
##       봉우리의 90% 아래  0.32초
##       봉우리의 70% 아래  0.46초
##       봉우리의 50% 아래  0.86초   <- 이 값
##
## **모으는 시간이 이 길이입니다**(player.gd 의 `SHOUT_CHARGE_TIME`).
const SHOUT_LEN := 0.86
