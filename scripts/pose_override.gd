...
## 풀장 **테두리 위에** 걸터앉은 자세(물물교환하는 아이).
##
## 처음에는 물 안에 기대앉히려 했는데, 재 보니 **테두리가 1.20m** 라 아이 키
## (1.25m)와 거의 같았습니다 - 안에 앉으면 무엇을 해도 테두리에 가립니다.
## 위에 걸터앉으면 온몸이 보이고, 다리를 물 쪽으로 늘어뜨릴 수 있습니다.
##
## 팔은 **아래로** 내려 옆에 둡니다(테두리를 짚는 모양). 다리는 조금 벌리고
## 무릎을 굽혀 물 쪽으로 늘어뜨립니다 - 그 무릎을 걷듯이 폈다 굽혔다 하는
## 것은 `shopkeeper.gd` 가 매 프레임 씁니다.
##
## 각도 규약은 위와 같습니다: X 양수가 앞, Z 는 좌우로 벌리는 축(좌우 거울).
const POOL_SIT := {
	# 앉은 상체. 살짝만 뒤로 - 테두리를 짚고 몸을 뒤로 맡긴 모양입니다.
	"Spine": Vector3(-10, 0, 0), "Chest": Vector3(-6, 0, 0),
	# 고개는 앞을 봅니다. 화면 쪽을 보고 앉아 있어야 말을 걸 상대로 읽힙니다.
	"Neck": Vector3(-4, 0, 0), "Head": Vector3(-2, 0, 0),
	# **팔은 아래로.** 옆으로 조금만 벌려 테두리를 짚습니다. 뒤로 살짝
	# 보내야(X 음수) 몸을 맡긴 것으로 보입니다.
	"LeftArm": Vector3(-16, 0, 12), "RightArm": Vector3(-16, 0, -12),
	"LeftForeArm": Vector3(8, 0, 0), "RightForeArm": Vector3(8, 0, 0),
	# 걸터앉은 다리. 허벅지를 앞으로 들어 수평에 가깝게, 무릎을 크게 굽혀
	# 종아리를 아래로 늘어뜨립니다. Z 로 조금 벌립니다.
	"LeftUpLeg": Vector3(86, 0, 11), "RightUpLeg": Vector3(86, 0, -11),
	"LeftLeg": Vector3(-78, 0, 0), "RightLeg": Vector3(-78, 0, 0),
	"LeftFoot": Vector3(12, 0, 0), "RightFoot": Vector3(12, 0, 0),
}
## 무릎을 폈다 굽히는 폭(도)과 그 한가운데 값.
##
## 걷듯이 번갈아 움직입니다. 폭이 크면 발버둥이고, 작으면 굳은 인형입니다 -
## 22도면 종아리 끝이 한 뼘쯤 오갑니다.
const POOL_KNEE := -78.0
const POOL_KNEE_SWING := 22.0
