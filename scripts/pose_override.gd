...
const ROLL := {
	...
	"Neck": Vector3(22, 0, 0),
	"Head": Vector3(16, 0, 0),
	"LeftArm": Vector3(78, -34, -10), "RightArm": Vector3(78, 34, 10),
...
## 아니라 들이붓는 것이 됩니다.
##
## **팔꿈치를 접는 축을 고쳤습니다.** 전에는 팔뚝 X 를 135 도 접어서 손이
## 얼굴 앞으로 오게 했는데, 그건 팔뚝을 **수평으로** 돌리는 축이라 손끝이
## 옆·뒤를 가리켰습니다(실측 팔뚝 방향 (-0.78, 0.17, 0.60) - 하늘 성분이
## 0.17 뿐입니다). 잔을 쥔 손은 손끝이 위를 봐야 합니다.
##
## 접는 축은 **Z** 이고 **좌우 부호가 반대**입니다(∓90). 같은 부호를 주면
## 한 손만 하늘을, 다른 손은 땅을 가리킵니다 - 실제로 그렇게 나왔습니다.
##
## 벌림은 어깨 Y 가 정합니다. 우유갑은 두 손 사이에 놓이므로 **모아야**
## 합니다 - Y -42 에서 두 손 사이가 어깨의 0.43배, 손은 머리뼈보다 0.07m
## 위(입가)입니다.
##
##   팔뚝 X 30~135  -> 방향 Y 0.04~0.19   손끝이 옆
##   팔뚝 Z ∓90     -> 방향 Y **0.84**    손끝이 하늘
const DRINK := {
	"Spine": Vector3(-6, 0, 0), "Chest": Vector3(-4, 0, 0),
	"Neck": Vector3(-18, 0, 0), "Head": Vector3(-14, 0, 0),
	"LeftArm": Vector3(80, -42, 0), "RightArm": Vector3(80, 42, 0),
	"LeftForeArm": Vector3(0, 0, -90), "RightForeArm": Vector3(0, 0, 90),
}
...
## 무엇을 하는지 전달되지 않습니다.
##
## **쓰는 쪽은 물물교환하는 아이입니다**(`shopkeeper.gd`). 주인공이 물에
## 들어가던 시절의 자세인데, 풀장이 곧 상점이 되면서 주인공은 들어갈 일이
## 없어졌고 아이가 그 자리에 섰습니다.
const SPLASH_A := {
...
## **발 걸기.** 받아 낸 뒤 옆으로 비켜서면서 발을 뻗어 넘어뜨립니다.
##
## 막기(`BLOCK`)와 짝입니다 - 막는 자세에서 그대로 이어지므로 상체는 비슷하게
## 두되(팔은 조금 내려 균형을 잡습니다), **다리가 반대**입니다: 막기는 오른
## 무릎을 접어 몸을 낮추고, 여기서는 그 발을 **앞으로 쭉 뻗습니다.**
##
## 왼 다리는 접어 몸을 낮춥니다. 뻗은 발이 바닥에 붙으려면 반대쪽이 굽어야
## 하고, 낮아진 몸이 곧 「걸었다」 는 그림입니다.
##
## 허리를 옆으로 트는 것(Spine Y)이 **비켜서는 것**을 말합니다. 앞뒤로만
## 접으면 그냥 웅크린 모양이라 옆으로 빠지는 것이 안 읽힙니다.
const TRIP := {
	"Spine": Vector3(14, 22, 0), "Chest": Vector3(8, 14, 0),
	"Neck": Vector3(10, -10, 0), "Head": Vector3(8, -8, 0),
	"LeftArm": Vector3(52, -28, 0), "RightArm": Vector3(52, 28, 0),
	"LeftForeArm": Vector3(0, 0, -46), "RightForeArm": Vector3(0, 0, 46),
	"RightUpLeg": Vector3(88, 0, 0), "RightLeg": Vector3(-4, 0, 0),
	"LeftUpLeg": Vector3(28, 0, 0), "LeftLeg": Vector3(-74, 0, 0),
}


## **막기.** 주인공이 받아 내려고 웅크리는 자세입니다.
##
## 적의 `GUARD`(베개로 막고 선 것)와 이름만 비슷하고 하는 일이 다릅니다 -
## 저쪽은 **늘** 그 자세이고, 이쪽은 **0.3초 남짓**만 나옵니다. 짧게 스치는
## 자세라 실루엣이 한눈에 달라야 합니다.
##
## | 무엇 | 어떻게 | 왜 |
## |---|---|---|
## | 오른 다리 | 허벅지를 앞으로, 무릎을 접음 | 몸이 낮아집니다 |
## | 왼 다리 | 뒤로 **일자**로 뻗음 | 낮아진 자세를 뒤에서 받칩니다 |
## | 두 팔 | 얼굴을 감싸고 팔목을 90도로 | 받아 내는 것은 **얼굴을 지키는 것**입니다 |
## | 허리 | 아주 살짝만 | 크게 접으면 「맞겠다」 가 되지 「받아친다」 가 아닙니다 |
## | 고개 | 조금 더 낮춤 | 팔 사이로 앞을 봅니다 |
##
## **팔 각도는 재서 골랐습니다**(`--pose=block`). 머리가 큰 아이라 팔을 앞으로만
## 올리면 두 손이 얼굴 한가운데에서 겹칩니다 - 처음 값(Y 0)에서는 **두 손 사이가
## 3.8cm** 였습니다. 벌리는 것은 Y 와 Z 가 같이 합니다.
##
...
##
## 머리뼈는 0.91, 머리 한가운데는 1.03 입니다(뼈는 목 위에 있고 머리통은 그보다
## 위·앞으로 큽니다 - **뼈까지의 거리로 재면 얼굴을 뚫고 있어도 「0.2m 떨어짐」
## 으로 읽힙니다**).
const BLOCK := {
	"Spine": Vector3(9, 0, 0), "Chest": Vector3(6, 0, 0),
	"Neck": Vector3(14, 0, 0), "Head": Vector3(10, 0, 0),
	"LeftArm": Vector3(78, -14, 0), "RightArm": Vector3(78, 14, 0),
	"LeftForeArm": Vector3(0, 0, -90), "RightForeArm": Vector3(0, 0, 90),
	"RightUpLeg": Vector3(56, 0, 0), "RightLeg": Vector3(-72, 0, 0),
	"LeftUpLeg": Vector3(-36, 0, 0), "LeftLeg": Vector3(0, 0, 0),
}
