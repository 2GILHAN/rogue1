...
var shout_stun := 0.0       # 고함이 굳히는 시간(초). 넉백을 대신합니다
...
func _recompute() -> void:
	...
	#
	# **밀어내는 힘을 없앴습니다.** 고함이 하는 일은 이제 「끊는 것」입니다
	# (player.gd 의 `_shout_apply` 참고) - 밀어내면 끊어 놓고 멀리 보내는
	# 셈이라, 정리한 뒤 때리는 일(밀기)과 이어지지 않았습니다. 굳히는 시간이
	# 단계마다 늘어납니다.
	shout_range = [0.0, 1.0, 2.0, 3.0][ls]
	shout_stun = [0.0, 1.0, 1.4, 1.8][ls]
