...
## 반마다 한 줄. **그 반에서 처음 만나는 것**을 적습니다.
##
## 이름만 뜨면 "몇 번째인가" 밖에 안 알려 줍니다. 무엇이 새로 나오는지가
## 붙으면 들어가기 전에 마음의 준비를 할 수 있고, 다섯 반이 서로 다른 곳으로
## 기억됩니다.
const FLOOR_NOTES := [
	"처음 오는 반. 박치기하는 아이들뿐입니다.",
	"던지는 아이와 고함치는 아이가 있습니다.",
	"베개를 든 아이가 문을 지킵니다.",
	"매달리는 아이가 붙습니다. 떼어 내야 합니다.",
	"선생님이 기다립니다. 여기가 마지막입니다.",
]


static func floor_note(n: int) -> String:
	if n >= 1 and n <= FLOOR_NOTES.size():
		return FLOOR_NOTES[n - 1]
	return ""
...
var parry_window := 0.16    # 고함을 누른 뒤 **패링으로 받아 내는** 시간(초)
...
func _recompute() -> void:
	...
	# 고함. 사거리와 **패링 판정 창**이 오릅니다. 피해는 계통과 무관하게
	# 처음부터 밀기의 절반입니다(`Player.SHOUT_MULT`).
	#
	# **계통이 올린다고 세지지 않습니다. 쉬워집니다.** 고함의 값은 이제
	# 「맞는 순간에 맞춰 누르는 것」이고(player.gd 의 패링), 그 판정 창이
	# 넓어지는 것이 곧 보상입니다 - 어려운 기술을 파면 손에 붙습니다.
	#
	# 계통을 안 팠어도 0.16 초는 있습니다. 아예 0 이면 이 기술을 배울 자리가
	# 없어서, 무엇을 파야 좋은지도 알 수 없습니다.
	shout_range = [0.0, 1.0, 2.0, 3.0][ls]
	parry_window = [0.16, 0.22, 0.28, 0.34][ls]
...
func has_shout_damage() -> bool:
	## 고함 Lv3. **이름이 남아 있지만 피해와는 상관없습니다.**
	##
	## 한동안 Lv3 전까지 고함의 피해가 0 이었습니다. 손에는 「안 아픈 버튼」
	## 으로만 남아서 걷어냈습니다 - 지금 고함은 **처음부터** 밀기의 절반만큼
	## 아픕니다(`Player.SHOUT_MULT`). 이 함수는 계통을 끝까지 판 표시(호랑이
	## 이펙트·구슬)에만 씁니다.
